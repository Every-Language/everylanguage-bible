...
const ExpandedMediaContent: React.FC<ExpandedMediaContentProps> = ({
  ...
  // Helper function to generate image path from book name
  const getImagePathFromBookName = (bookName: string): string | null => {
    if (!bookName) return null;

    // Find the book order from a hardcoded list (fallback when store isn't ready)
    ...
    const order = bookOrderMap[bookName];
    ...
    const fileName =
      nameMap[bookName] || bookName.toLowerCase().replace(/\s+/g, '-');
  ...
  const getBookWithImagePath = (
    ...
    // If books aren't loaded yet, try to initialize them immediately
    if (bibleBooks.length === 0) {
      initializeBibleBooks();
    }

    // If books are available, try to find the book
    if (bibleBooks.length > 0) {
      // Try exact match first
      let book = bibleBooks.find(b => b.name === bookName);

      // If no exact match, try case-insensitive match
      if (!book) {
        book = bibleBooks.find(
    ...
    // Fallback: try to generate image path directly based on book name
    // This handles cases where the store isn't initialized yet
